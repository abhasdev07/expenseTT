...
def get_summary():
    ...
    try:
        ...
        current_user_id = get_user_id_from_jwt(jwt_identity)
        ...
        total_expenses = expense_total.scalar() or Decimal("0")

        print(
            f"DEBUG: Calculated totals - Income: {total_income}, Expenses: {total_expenses}"
        )

        # Transaction counts
        income_count = Transaction.query.filter_by(
            user_id=current_user_id, type="income"
        )
        expense_count = Transaction.query.filter_by(
            user_id=current_user_id, type="expense"
        )
        ...
        print(f"DEBUG: Summary being returned: {summary}")
        dumped = summary_schema.dump(summary)
        print(f"DEBUG: After schema dump: {dumped}")
...
def get_spending_by_category():
    ...
    try:
            ...
            spending_data.append(
                {
                    ...
                    "percentage": round((float(result.total_amount) / total * 100), 2)
                    if total > 0
                    else 0,
                }
            )
        ...
        return jsonify(
            {"error": "Failed to fetch spending data", "message": str(e)}
        ), 500
...
def get_trends():
    ...
    try:
        ...
        return jsonify({"trends": trend_schema.dump(trends), "period": period}), 200
...
def get_insights():
    ...
    try:
        ...
        budgets = Budget.query.filter_by(
            user_id=current_user_id, month=current_month, year=current_year
        ).all()
        ...
        return jsonify({"insights": insight_schema.dump(insights)}), 200
